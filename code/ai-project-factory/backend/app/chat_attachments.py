"""Project-scoped chat images. Binary storage is separate from message history."""
import base64
import binascii
import io
...
def bind_attachments(db, identity, message_id, ids):
    for attachment_id in ids:
        ...
        if not row or row.design_id != identity or row.message_id is not None:
            raise HTTPException(422, "图片不存在、属于其他项目或已用于另一条消息，请重新上传")
...
def install_chat_attachments(app, sessions):
    ...
    def upload(identity: str, body: UploadImage):
        with sessions.begin() as db:
            ...
            try:
                ...
                raise HTTPException(422, "请上传有效的 PNG、JPEG 或 WebP 图片，单张不超过 5 MB、2500 万像素")
            row = ChatAttachment(design_id=identity, name=body.name, content_type=Image.MIME[kind], data=data)
            db.add(row)
            db.flush()
            return describe(row)
    ...
    def remove(identity: str, attachment_id: str):
        with sessions.begin() as db:
            ...
            if not row or row.design_id != identity:
                raise HTTPException(404, "图片不存在")
            if row.message_id:
                raise HTTPException(409, "已发送图片属于对话历史，不能从附件栏删除")
            db.delete(row)
            return {"deleted": True}
