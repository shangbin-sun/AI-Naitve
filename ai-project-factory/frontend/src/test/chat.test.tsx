...
describe("AI 团队对话", () => {
  ...
  it("Markdown 无 MIME 时可上传，待发送与历史均不生成图片", async () => {
    ...
    const p = props();
    const view = render(<ProjectChat {...p} />);
    fireEvent.paste(screen.getByLabelText("讨论当前AI 团队"), {
      clipboardData: {
  ...
  it("空消息不可发送，Enter 发送文字并清空输入", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText("讨论当前AI 团队"),
      "保留员工并调整流程{Enter}",
    );
    await waitFor(() =>
  ...
  it("发送被拒绝时恢复输入，支持再次发送", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("讨论当前AI 团队")).toHaveValue("修改测试节点"),
    );
    await userEvent.click(screen.getByRole("button", { name: "发送" }));
  ...
  it("Shift Enter 换行，中文输入法确认不会提交", async () => {
    const p = props();
    render(<ProjectChat {...p} />);
    const input = screen.getByLabelText("讨论当前AI 团队");
  ...
  it("运行时显示进度并可停止，构建按钮保留", async () => {
    ...
    expect(screen.getByLabelText("讨论当前AI 团队")).toBeDisabled();
  ...
  it("恢复历史文字和图片；按AI 团队 key 切换隔离草稿", async () => {
    const p = props();
    const view = render(
      <ProjectChat
        key="one"
    ...
    await userEvent.type(screen.getByLabelText("讨论当前AI 团队"), "未发送的修改");
  ...
  it("图片上传适配器保存服务端 ID，移除调用AI 团队接口", async () => {
...
it("未引用员工时直接输入，不要求手动选择目标", () => {
  ...
  expect(screen.getByLabelText("讨论当前AI 团队")).toBeEnabled();
});
